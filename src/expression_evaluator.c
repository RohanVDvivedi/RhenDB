...
static int et_is_int   (expr_type t){ return t==RHENDB_BIT_FIELD||t==RHENDB_UINT||t==RHENDB_INT||t==RHENDB_LARGE_UINT||t==RHENDB_LARGE_INT; }
static int et_is_num   (expr_type t){ return et_is_int(t)||t==RHENDB_FLOAT; }
static int et_is_large (expr_type t){ return t==RHENDB_LARGE_UINT||t==RHENDB_LARGE_INT; }
static int et_is_signed(expr_type t){ return t==RHENDB_INT||t==RHENDB_LARGE_INT; }
static int et_is_num_or_numeric(expr_type t){ return et_is_num(t) || t==RHENDB_NUMERIC; }
...
{
	if(a==RHENDB_NUMERIC || b==RHENDB_NUMERIC) return RHENDB_NUMERIC;
	if(a==RHENDB_FLOAT || b==RHENDB_FLOAT) return RHENDB_FLOAT;
	int signd = et_is_signed(a) || et_is_signed(b);
	if(et_is_large(a) || et_is_large(b)) return signd ? RHENDB_LARGE_INT : RHENDB_LARGE_UINT;
	return signd ? RHENDB_INT : RHENDB_UINT;
}
 
static expr_value*     new_val (expr_type t){ expr_value* v = calloc(1, sizeof *v); v->type_info.type = t; return v; }
static expr_type_info* new_type(expr_type t){ expr_type_info* ti = calloc(1, sizeof *ti); ti->type = t; return ti; }
 
/* discriminators */
static int is_materialized_numeric(const expr_value* v){ return v->type_info.type == RHENDB_NUMERIC && v->type_info.dti_p == NULL; }
static int is_tuple_numeric       (const expr_value* v){ return v->type_info.dti_p != NULL && is_numeric_type_info(v->type_info.dti_p); }
static int is_numeric_operand     (const expr_value* v){ return is_materialized_numeric(v) || is_tuple_numeric(v); }
static int is_native_float(const expr_value* v, double* out){ if(v->type_info.dti_p == NULL && v->type_info.type == RHENDB_FLOAT){ *out = v->value.double_value; return 1; } return 0; }
...
{
	if(v->type_info.dti_p != NULL) return is_text_type_info(v->type_info.dti_p) || is_blob_type_info(v->type_info.dti_p);
...
static double  to_dbl(const expr_value* v){
	switch(v->type_info.type){
		case RHENDB_BIT_FIELD: case RHENDB_UINT: return (double)v->value.uint_value;
		case RHENDB_INT: return (double)v->value.int_value;
		case RHENDB_FLOAT: return v->value.double_value;
		case RHENDB_LARGE_UINT: return convert_to_double_uint256(v->value.large_uint_value);
		case RHENDB_LARGE_INT: return convert_to_double_int256(v->value.large_int_value);
		default: return 0;
	}
}
static uint64_t to_u64(const expr_value* v){ return (v->type_info.type==RHENDB_INT) ? (uint64_t)v->value.int_value : v->value.uint_value; }
static int64_t  to_i64(const expr_value* v){ return (v->type_info.type==RHENDB_INT) ? v->value.int_value : (int64_t)v->value.uint_value; }
static uint256  to_u256(const expr_value* v){
	switch(v->type_info.type){
		case RHENDB_BIT_FIELD: case RHENDB_UINT: return get_uint256(v->value.uint_value);
		case RHENDB_INT: return get_uint256((uint64_t)v->value.int_value);
		case RHENDB_LARGE_UINT: return v->value.large_uint_value;
		case RHENDB_LARGE_INT: return v->value.large_int_value.raw_uint_value;
		default: return get_uint256(0);
	}
}
static int256 to_i256(const expr_value* v){
	switch(v->type_info.type){
		case RHENDB_BIT_FIELD: case RHENDB_UINT: return (int256){ get_uint256(v->value.uint_value) };
		case RHENDB_INT: return get_int256(v->value.int_value);
		case RHENDB_LARGE_UINT: return (int256){ v->value.large_uint_value };
		case RHENDB_LARGE_INT: return v->value.large_int_value;
		default: return get_int256(0);
	}
}
...
{
	if(ec_p == NULL || ec_p->context_p == NULL) return NULL;
	rhendb_expr_eval_context* ctx = ec_p->context_p;
	if(ctx->rdb == NULL) return NULL;
	return &(ctx->rdb->persistent_acid_rage_engine);
}
...
static int materialize_tb(expr_value* v, const sql_expr_eval_context* ec_p, int* error_code)
{
	...
	if(dti == NULL) return RHENDB_EE_OK;                    /* already native */
	int is_txt = is_text_type_info(dti);
	int is_bin = is_blob_type_info(dti);
	if(!is_txt && !is_bin) return RHENDB_EE_OK;             /* not text/blob */
	expr_type target = is_txt ? RHENDB_STRING : RHENDB_BINARY;
 
	rage_engine* eng = engine_from_ctx(ec_p);
	if(eng == NULL){ *error_code = RHENDB_EE_MISSING_ENGINE; return *error_code; }
 
	const void* transaction_id = NULL;
	int abort_error = 0;
	binary_read_iterator* bri = get_new_binary_read_iterator(&(v->value), dti, &(eng->bstd), eng->pam_p);
	if(bri == NULL){ *error_code = RHENDB_EE_MATERIALIZE_FAILED; return *error_code; }
 
	uint32_t cap = 64, len = 0;
	char* buf = malloc(cap);
	if(buf == NULL){ delete_binary_read_iterator(bri, transaction_id, &abort_error); *error_code = RHENDB_EE_OUT_OF_MEMORY; return *error_code; }
	while(1)
	{
		if(len == cap){ uint32_t nc = cap * 2; char* nb = realloc(buf, nc); if(nb == NULL){ free(buf); delete_binary_read_iterator(bri, transaction_id, &abort_error); *error_code = RHENDB_EE_OUT_OF_MEMORY; return *error_code; } buf = nb; cap = nc; }
		uint32_t got = read_from_binary_read_iterator(bri, buf + len, cap - len, transaction_id, &abort_error);
		if(abort_error){ free(buf); delete_binary_read_iterator(bri, transaction_id, &abort_error); *error_code = RHENDB_EE_MATERIALIZE_FAILED; return *error_code; }
		if(got == 0) break;
		len += got;
	}
	delete_binary_read_iterator(bri, transaction_id, &abort_error);
	if(abort_error){ free(buf); *error_code = RHENDB_EE_MATERIALIZE_FAILED; return *error_code; }
 
	if(v->type_info.should_free_dti_p && v->type_info.dti_p) destroy_type_info_recursively(v->type_info.dti_p, NULL);
	if(v->buffer) free(v->buffer);
	v->buffer = buf; v->capacity = len;
	v->value = (datum){0};
	v->value.string_or_binary_value = buf;
	v->value.string_or_binary_size = len;
...
{
	if(v->type_info.dti_p == NULL) return RHENDB_EE_OK;                 /* already native */
	if(!is_numeric_type_info(v->type_info.dti_p)) return RHENDB_EE_OK;  /* not a numeric */
	...
	if(abort_error){ *error_code = RHENDB_EE_MATERIALIZE_FAILED; return *error_code; }
 
	numeric_sign_bits sb; int16_t exp;
	nri.extract_sign_bits_and_exponent(&nri, &sb, &exp);
 
	materialized_numeric mn;
	if(!initialize_materialized_numeric(&mn, 8)){ nri.close_digits_stream(&nri); *error_code = RHENDB_EE_OUT_OF_MEMORY; return *error_code; }
	...
	if(sb == POSITIVE_NUMERIC || sb == NEGATIVE_NUMERIC)   /* only finite non-zero values carry digits */
	{
		...
		while(1)
		{
			...
			if(err){ nri.close_digits_stream(&nri); deinitialize_materialized_numeric(&mn); *error_code = RHENDB_EE_MATERIALIZE_FAILED; return *error_code; }
			if(got == 0) break;
			/* digits stream MSD-first; push_lsd appends, keeping the MSD at the front */
			for(uint32_t i = 0; i < got; i++) push_lsd_in_materialized_numeric(&mn, buf[i]);
		}
	}
	...
	if(v->type_info.should_free_dti_p && v->type_info.dti_p) destroy_type_info_recursively(v->type_info.dti_p, NULL);
	if(v->buffer){ free(v->buffer); v->buffer = NULL; v->capacity = 0; }
...
{
	if(!ee_mpd_new(out)) return 0;
	mpd_context_t ctx; mpd_maxcontext(&ctx); uint32_t st = 0;
	char b[64];
	switch(v->type_info.type){
		case RHENDB_BIT_FIELD: case RHENDB_UINT: mpd_qset_u64(out, v->value.uint_value, &ctx, &st); break;
		case RHENDB_INT: mpd_qset_i64(out, v->value.int_value, &ctx, &st); break;
		case RHENDB_FLOAT:      snprintf(b, sizeof b, "%.17g", v->value.double_value); mpd_qset_string(out, b, &ctx, &st); break;
		case RHENDB_LARGE_UINT: snprintf(b, sizeof b, "%.17g", convert_to_double_uint256(v->value.large_uint_value)); mpd_qset_string(out, b, &ctx, &st); break;
		case RHENDB_LARGE_INT:  snprintf(b, sizeof b, "%.17g", convert_to_double_int256(v->value.large_int_value));  mpd_qset_string(out, b, &ctx, &st); break;
		default: mpd_del(out); return 0;
	}
	return 1;
}
...
{
	*owns = 0;
	if(is_materialized_numeric(v)) return &(v->numeric_value);
	if(is_tuple_numeric(v)){
		if(materialize_numeric(v, ec_p, error_code)) return NULL;
		return &(v->numeric_value);
	}
	if(et_is_num(v->type_info.type) && v->type_info.dti_p == NULL){
		if(!number_to_mpd(v, scratch)){ *error_code = RHENDB_EE_OUT_OF_MEMORY; return NULL; }
		*owns = 1;
		return scratch;
	}
...
typedef enum { OP_ADD, OP_SUB, OP_MUL, OP_DIV, OP_MOD } arith_op;
...
static void* do_arith(void* d1, void* d2, arith_op op, const sql_expr_eval_context* ec_p, int* error_code)
{
	...
	if(is_numeric_operand(a) || is_numeric_operand(b))
	{
		...
		if((op == OP_DIV || op == OP_MOD) && mpd_iszero(pb)){
			if(oa) mpd_del(&sa);
			if(ob) mpd_del(&sb);
			*error_code = RHENDB_EE_DIVIDE_BY_ZERO; return NULL;
		}
		...
		expr_value* v = new_val(RHENDB_NUMERIC); v->numeric_value = result; return v;
	}
 
	/* non-numeric operands must be plain in-memory numbers */
	expr_type ta = a->type_info.type, tb = b->type_info.type;
	if(a->type_info.dti_p != NULL || b->type_info.dti_p != NULL || !et_is_num(ta) || !et_is_num(tb)){ *error_code = RHENDB_EE_NON_NUMERIC_OPERAND; return NULL; }
	expr_type rt = num_result(ta, tb);
 
	if(rt == RHENDB_FLOAT){
		double x = to_dbl(a), y = to_dbl(b), r = 0;
		switch(op){
			case OP_ADD: r = x + y; break; case OP_SUB: r = x - y; break; case OP_MUL: r = x * y; break;
			case OP_DIV: if(y == 0){ *error_code = RHENDB_EE_DIVIDE_BY_ZERO; return NULL; } r = x / y; break;
			case OP_MOD: if(y == 0){ *error_code = RHENDB_EE_DIVIDE_BY_ZERO; return NULL; } r = fmod(x, y); break;
		}
		expr_value* v = new_val(RHENDB_FLOAT); v->value.double_value = r; return v;
	}
	if(rt == RHENDB_LARGE_INT){
		int256 x = to_i256(a), y = to_i256(b), r; int256 q;
		switch(op){
			case OP_ADD: add_int256(&r, x, y); break; case OP_SUB: sub_int256(&r, x, y); break; case OP_MUL: mul_int256(&r, x, y); break;
			case OP_DIV: if(is_zero_int256(y)){ *error_code = RHENDB_EE_DIVIDE_BY_ZERO; return NULL; } div_int256(&r, x, y); break;
			case OP_MOD: if(is_zero_int256(y)){ *error_code = RHENDB_EE_DIVIDE_BY_ZERO; return NULL; } { int256 rem = div_int256(&q, x, y); r = rem; } break;
		}
		expr_value* v = new_val(RHENDB_LARGE_INT); v->value.large_int_value = r; return v;
	}
	if(rt == RHENDB_LARGE_UINT){
		uint256 x = to_u256(a), y = to_u256(b), r;
		switch(op){
			case OP_ADD: add_uint256(&r, x, y); break; case OP_SUB: sub_uint256(&r, x, y); break; case OP_MUL: mul_uint256(&r, x, y); break;
			case OP_DIV: if(is_zero_uint256(y)){ *error_code = RHENDB_EE_DIVIDE_BY_ZERO; return NULL; } div_uint256(&r, x, y); break;
			case OP_MOD: if(is_zero_uint256(y)){ *error_code = RHENDB_EE_DIVIDE_BY_ZERO; return NULL; } { uint256 q; uint256 rem = div_uint256(&q, x, y); r = rem; } break;
		}
		expr_value* v = new_val(RHENDB_LARGE_UINT); v->value.large_uint_value = r; return v;
	}
	if(rt == RHENDB_INT){
		...
		switch(op){
			case OP_ADD: r = x + y; break; case OP_SUB: r = x - y; break; case OP_MUL: r = x * y; break;
			case OP_DIV: if(y == 0){ *error_code = RHENDB_EE_DIVIDE_BY_ZERO; return NULL; } r = x / y; break;
			case OP_MOD: if(y == 0){ *error_code = RHENDB_EE_DIVIDE_BY_ZERO; return NULL; } r = x % y; break;
		}
		expr_value* v = new_val(RHENDB_INT); v->value.int_value = r; return v;
	}
	{ /* RHENDB_UINT */
		...
		expr_value* v = new_val(RHENDB_UINT); v->value.uint_value = r; return v;
	}
}
...
{
	rage_engine* eng = engine_from_ctx(ec_p);
	if(eng == NULL){ *error_code = RHENDB_EE_MISSING_ENGINE; return 0; }
	...
	if(bri == NULL){ *error_code = RHENDB_EE_MATERIALIZE_FAILED; return 0; }
	char one; uint32_t got = read_from_binary_read_iterator(bri, &one, 1, transaction_id, &abort_error);
	delete_binary_read_iterator(bri, transaction_id, &abort_error);
	if(abort_error){ *error_code = RHENDB_EE_MATERIALIZE_FAILED; return 0; }
	return got == 0;
}
...
static void* rhendb_get_bool(void* data, const sql_expr_eval_context* ec_p, int* error_code)
{
	...
	int truthy;
	switch(v->type_info.type){
		case RHENDB_BIT_FIELD: case RHENDB_UINT: truthy = (v->value.uint_value != 0); break;
		case RHENDB_INT: truthy = (v->value.int_value != 0); break;
		case RHENDB_FLOAT: truthy = (v->value.double_value != 0); break;   /* NaN != 0 -> true */
		case RHENDB_LARGE_UINT: truthy = !is_zero_uint256(v->value.large_uint_value); break;
		case RHENDB_LARGE_INT: truthy = !is_zero_int256(v->value.large_int_value); break;
		case RHENDB_STRING: truthy = (v->value.string_size != 0); break;
		case RHENDB_BINARY: truthy = (v->value.binary_size != 0); break;
...
{
	if(v->type_info.dti_p != NULL) return v->type_info.dti_p;
	switch(v->type_info.type){
...
static int rhendb_compare(void* data1, void* data2, const sql_expr_eval_context* ec_p, int* error_code)
{
	...
	if(is_numeric_operand(a) || is_numeric_operand(b)){
		...
		if(an || bn) r = (an && bn) ? 0 : (an ? 1 : -1);
		else { uint32_t st = 0; int c = mpd_qcmp(pa, pb, &st); r = (c > 0) ? 1 : (c < 0) ? -1 : 0; }
		if(oa) mpd_del(&sa);
		if(ob) mpd_del(&sb);
		return r;
	}
 
	/* FLOAT NaN rule : NaN == NaN and NaN is greater than every other value */
	{
		double fa, fb; int af = is_native_float(a, &fa), bf = is_native_float(b, &fb);
		int a_nan = af && isnan(fa), b_nan = bf && isnan(fb);
		if(a_nan || b_nan){ if(a_nan && b_nan) return 0; return a_nan ? 1 : -1; }
	}
	...
	if(a_sb && b_sb){
		...
		if(a->type_info.dti_p != NULL && b->type_info.dti_p != NULL){
			if(!can_compare_datum_rhendb(a->type_info.dti_p, b->type_info.dti_p)){ *error_code = RHENDB_EE_INCOMPARABLE_TYPES; return 0; }
	...
	if(a_sb || b_sb){ *error_code = RHENDB_EE_INCOMPARABLE_TYPES; return 0; }   /* string vs number */
	...
	if(d1 == NULL || d2 == NULL || !can_compare_datum_rhendb(d1, d2)){ *error_code = RHENDB_EE_INCOMPARABLE_TYPES; return 0; }
	return compare_datum_rhendb(&a->value, d1, &b->value, d2, engine_from_ctx(ec_p));
}
 
/* ------------------------------ bitwise / shifts ------------------------------ */
 
typedef enum { B_AND, B_OR, B_XOR } bit_op;
static void* do_bitlogic(void* d1, void* d2, bit_op op, int* error_code)
{
	expr_value* a = d1; expr_value* b = d2;
	expr_type ta = a->type_info.type, tb = b->type_info.type;
	if(a->type_info.dti_p != NULL || b->type_info.dti_p != NULL || !et_is_int(ta) || !et_is_int(tb)){ *error_code = RHENDB_EE_NON_INTEGER_OPERAND; return NULL; }
	expr_type rt = num_result(ta, tb);
	if(et_is_large(rt)){
		uint256 x = to_u256(a), y = to_u256(b), r;
		r = (op==B_AND) ? bitwise_and_uint256(x,y) : (op==B_OR) ? bitwise_or_uint256(x,y) : bitwise_xor_uint256(x,y);
		expr_value* v = new_val(rt);
		if(rt==RHENDB_LARGE_INT) v->value.large_int_value = (int256){ r }; else v->value.large_uint_value = r;
		return v;
	}
	uint64_t x = to_u64(a), y = to_u64(b), r;
	r = (op==B_AND) ? (x & y) : (op==B_OR) ? (x | y) : (x ^ y);
	expr_value* v = new_val(rt);
	if(rt==RHENDB_INT) v->value.int_value = (int64_t)r; else v->value.uint_value = r;
	return v;
}
...
static void* rhendb_bit_not(void* data, const sql_expr_eval_context* ec_p, int* error_code)
{
	(void)ec_p; expr_value* a = data;
	expr_type t = a->type_info.type;
	if(a->type_info.dti_p != NULL || !et_is_int(t)){ *error_code = RHENDB_EE_NON_INTEGER_OPERAND; return NULL; }
	expr_value* v = new_val(t);
	switch(t){
		case RHENDB_LARGE_UINT: v->value.large_uint_value = bitwise_not_uint256(a->value.large_uint_value); break;
		case RHENDB_LARGE_INT:  v->value.large_int_value  = (int256){ bitwise_not_uint256(a->value.large_int_value.raw_uint_value) }; break;
		case RHENDB_INT: v->value.int_value = ~a->value.int_value; break;
		default: v->value.uint_value = ~to_u64(a); break;
	}
	return v;
}
static void* do_shift(void* data, void* shift_amt, int left, int* error_code)
{
	expr_value* a = data; expr_value* s = shift_amt;
	expr_type t = a->type_info.type;
	if(a->type_info.dti_p != NULL || s->type_info.dti_p != NULL || !et_is_int(t) || !et_is_int(s->type_info.type)){ *error_code = RHENDB_EE_NON_INTEGER_OPERAND; return NULL; }
	uint32_t amt = (uint32_t)to_u64(s);
	expr_value* v = new_val(t);
	switch(t){
		case RHENDB_LARGE_UINT: v->value.large_uint_value = left ? left_shift_uint256(a->value.large_uint_value, amt) : right_shift_uint256(a->value.large_uint_value, amt); break;
		case RHENDB_LARGE_INT:  v->value.large_int_value  = (int256){ left ? left_shift_uint256(a->value.large_int_value.raw_uint_value, amt) : right_shift_uint256(a->value.large_int_value.raw_uint_value, amt) }; break;
		case RHENDB_INT: v->value.int_value = left ? (int64_t)(((uint64_t)a->value.int_value) << (amt & 63)) : (a->value.int_value >> (amt & 63)); break;
		default: v->value.uint_value = left ? (to_u64(a) << (amt & 63)) : (to_u64(a) >> (amt & 63)); break;
	}
	return v;
}
...
static void* rhendb_create_number(const dstring* data_bytes, const sql_expr_eval_context* ec_p, int* error_code)
{
	...
	if(strpbrk(buf, ".eE")){ expr_value* v = new_val(RHENDB_FLOAT); v->value.double_value = strtod(buf, NULL); return v; }
	expr_value* v = new_val(RHENDB_INT); v->value.int_value = strtoll(buf, NULL, 10); return v;
}
static void* rhendb_create_string(const dstring* data_bytes, const sql_expr_eval_context* ec_p, int* error_code)
{
	(void)ec_p; (void)error_code;
...
static void rhendb_concat(void** data1_p, void* data2, const sql_expr_eval_context* ec_p, int* error_code)
{
	...
	a->buffer = nbuf; a->capacity = na + nb;
	a->value = (datum){0};
	a->value.string_value = nbuf; a->value.string_size = na + nb;
...
static void* rhendb_like(void* str_p, void* pattern_p, const sql_expr_eval_context* ec_p, int* error_code)
{
	...
	if(s->type_info.type != RHENDB_STRING || p->type_info.type != RHENDB_STRING){ *error_code = RHENDB_EE_NON_STRING_OPERAND; return NULL; }
...
static void* rhendb_unify_types(void* typ1, void* typ2, const sql_expr_eval_context* ec_p, int* error_code)
{
	(void)ec_p;
	expr_type a = ((expr_type_info*)typ1)->type, b = ((expr_type_info*)typ2)->type;
	if(a == b) return new_type(a);
	if(et_is_num_or_numeric(a) && et_is_num_or_numeric(b)) return new_type(num_result(a, b));
	*error_code = RHENDB_EE_INCOMPARABLE_TYPES; return NULL;
}
...
static const hasher_interface     var_hasher     = simple_hasher(var_hash);
static const comparator_interface var_comparator = simple_comparator(var_cmp);
...
{
	if(is_numeric_type_info(dti)) return RHENDB_NUMERIC;
	if(is_text_type_info(dti))    return RHENDB_STRING;
	if(is_blob_type_info(dti))    return RHENDB_BINARY;
	switch(dti->type){
...
static int resolve_into(rhendb_expr_eval_context* ctx, const dstring* id,
                        uint32_t* out_tuple_index, positional_accessor* out_pa, const data_type_info** out_dti, int* error_code)
{
	...
	if(ncomp == 0){ *error_code = RHENDB_EE_UNSUPPORTED_TYPE; return 0; }
	...
	if(ctx->input_tuples_count >= 2){
		...
		if(found < 0){ *error_code = RHENDB_EE_UNSUPPORTED_TYPE; return 0; }   /* unknown table */
		tuple_index = (uint32_t)found; field_start = 1;
	}
	else {
	...
	if(field_start >= ncomp){ *error_code = RHENDB_EE_UNSUPPORTED_TYPE; return 0; }   /* no field named */
	if(ctx->input_tuple_defs[tuple_index] == NULL || ctx->input_tuple_defs[tuple_index]->type_info == NULL){ *error_code = RHENDB_EE_UNSUPPORTED_TYPE; return 0; }
	...
	*out_dti = get_type_info_for_element_from_tuple_def(ctx->input_tuple_defs[tuple_index], *out_pa);
	if(*out_dti == NULL) *out_dti = cur;
	return 1;
	#undef EE_MAX_PATH
}
...
static const var_cache_entry* get_or_resolve_entry(rhendb_expr_eval_context* ctx, const dstring* id, int* error_code)
{
	...
	if(ne == NULL){ *error_code = RHENDB_EE_OUT_OF_MEMORY; return NULL; }
	if(!init_dstring(&ne->identifier, get_byte_array_dstring(id), get_char_count_dstring(id))){ free(ne); *error_code = RHENDB_EE_OUT_OF_MEMORY; return NULL; }
 
	if(!resolve_into(ctx, id, &ne->tuple_index, &ne->pa, &ne->column_dti, error_code)){ deinit_dstring(&ne->identifier); free(ne); return NULL; }
 
	initialize_bstnode(&ne->node);
	if(!insert_in_hashmap(&ctx->var_cache, ne)){   /* red-black buckets never fill, so this only fails on a real error */
		free(ne->pa.positions); deinit_dstring(&ne->identifier); free(ne);
		*error_code = RHENDB_EE_OUT_OF_MEMORY; return NULL;
	}
	return ne;
}
...
static void* rhendb_get_variable(const dstring* identifier_bytes, const sql_expr_eval_context* ec_p, int* error_code)
{
	...
	if(d.is_NULL) return NULL;             /* NULL column -> NULL pointer */
 
	const data_type_info* dti = e->column_dti;
	expr_value* v = new_val(RHENDB_INT);
	v->value = d;
	v->buffer = NULL; v->capacity = 0;
	v->type_info.should_free_dti_p = 0;
	if(is_numeric_type_info(dti) || is_text_type_info(dti) || is_blob_type_info(dti)
	   || dti->type == TUPLE || dti->type == ARRAY){
...
static void free_var_cache_entry(void* resource_p, const void* data_p)
{
	(void)resource_p;
...
sql_expr_eval_context get_sql_expr_eval_context_for_rhendb(tuple_def** input_tuple_defs, uint32_t input_tuples_count, rhendb* rdb, void* catalog_manager)
{
	...
	initialize_hashmap(&(context_p->var_cache), ELEMENTS_AS_RED_BLACK_BST, 64, &var_hasher, &var_comparator, offsetof(var_cache_entry, node));
...
{
	remove_all_from_hashmap(&(context_p->var_cache), &((notifier_interface){NULL, free_var_cache_entry}));
