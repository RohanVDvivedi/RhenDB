...
static void execute(operator* o)
{
	...
	while(1)
	{
		...
		if(inputs->no_more_data[0] != inputs->no_more_data[1])
		{
			...
			{
				kill_signal_for_self_operator(o, get_dstring_pointing_to_literal_cstring("resuts_not_match_more_data_in_input_0"));
				return ;
			}
			else
			{
				kill_signal_for_self_operator(o, get_dstring_pointing_to_literal_cstring("resuts_not_match_more_data_in_input_1"));
				return ;
			}
		}
		...
		if(inputs->no_more_data[0] == 1)
		{
			...
			kill_signal_for_self_operator(o, get_dstring_pointing_to_literal_cstring("resuts_match_successfully"));
			return ;
		}
		...
		if(compare != 0)
		{
			...
			printf("\n\n");
			kill_signal_for_self_operator(o, get_dstring_pointing_to_literal_cstring("resuts_match_FAILED"));
			return ;
		}
...
static void clean_up_resources(operator* o)
{
	...
	if(inputs->input_iterators[0] != NULL)
	{
		destroy_consumption_iterator(inputs->input_iterators[0]);
		inputs->input_iterators[0] = NULL;
	}
	if(inputs->input_iterators[1] != NULL)
	{
		destroy_consumption_iterator(inputs->input_iterators[1]);
		inputs->input_iterators[1] = NULL;
	}
}
