...
transaction_status get_transaction_status_internal(void* NULL_p, uint256 transaction_id)
{
...
transaction_status get_transaction_status__(void* NULL_p, uint256 transaction_id)
{
	transaction_status res = get_transaction_status_internal(NULL_p, transaction_id);
	char temp[80] = {};
...
transaction_status_getter tsg = {NULL, get_transaction_status__};
...
int main()
{
...

	{
		...
		for(uint32_t xmin_i = 0; xmin_i < header_ids_count; xmin_i++)
		{
			...
			{
				mvcc_header hdr = {.is_xmin_NULL = are_equal_uint256(header_ids[xmax_i].transaction_id, get_uint256(0)), .xmin = header_ids[xmin_i], .is_xmax_NULL = are_equal_uint256(header_ids[xmax_i].transaction_id, get_uint256(0)), .xmax = header_ids[xmax_i]};
				int were_hints_updated = 0;
