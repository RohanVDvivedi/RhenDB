...
int main()
{
...

	{
		...
		// insert these rows into the table
		{
			...
			setup_insertion_operator(insertion_operator, input_operator, input_positions, ftbl, TABLE_ID_IN_OUTPUT | PARTITION_ID_IN_OUTPUT | TUPLE_POINTER_IN_OUTPUT | HEAP_TUPLE_IN_OUTPUT, NULL, SAVEPOINT_LOGGING_ENABLED);
	...
	// perform some peek and pop and print again
	{
		for(int l = 0; l < 2; l++)
		{
			...
			if(popped && type == NEW_SAVEPOINT_NAME_LOG)
				delete_savepoint(&tx, &savepoint_name);
			deinit_dstring(&savepoint_name);
		}
	}
	{
		printf("savepoints = %lu, savepoint_logs = %lu\b", tx.savepoint_logs.savepoint_names_count, tx.savepoint_logs.savepoint_logs_count);
